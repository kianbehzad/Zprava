...
void ZpSetting::initiate_networking(QList<QString>* list)
{
    ...
    npassword = list[0][1];
    username = list[0][2];
    request->setUrl(QUrl("http://127.0.0.1:8000/signup/setting/?nusername="+nusername+"&npassword="+npassword+"&username="+username));
...
{
    if(_reply == "Username Changed Successfully")
    {
        qDebug()<<_reply;
        delete WHOAMI;
        WHOAMI = new ZpUser(nusername);
    }
    else
    {
        qDebug()<< _reply;
